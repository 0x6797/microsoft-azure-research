...
## <a id="q-32kb">私のデバイスのメインメモリは32KBしかありませんが、IoT Hub を利用できますか？</a>

動作対象保証外です。すくなくとも 64KB 以上のメインメモリが必要です。

## <a id="q3">モジュールとは何ですか？</a>

モジュールとはデバイスに対する下位コンポーネントです。たとえば、装置がデバイスに対応し、装置に紐づくセンサ等がモジュールに対応します。

IoT Hub では、１つのデバイスに対して最大 20個のモジュールが設定可能です。
...
## <a id="q-pub-sub">MQTT や AMQP のパブリッシュ／サブスクライブ配信を利用できますか？</a>

IoT Hub は MQTT や AMQP のメッセージブローカーではないため、トピック等をしようした一般的なパブリッシュ／サブスクライブモデルを使用した M2M 通信の機能は提供されていません。したがって、デバイスから他のデバイスへの直接的な通信ではなく、一旦クラウド側でメッセージを受け取り、各デバイスへクラウドからメッセージを配信するという間接的な手順を踏む必要があります。

IoT Hub における MQTT および AMQP はデータの収集やクラウド側からのダイレクトメソッド呼び出しに対するレスポンス等に利用されます。

## <a id="q6">メッセージ以外にもデバイスからクラウドへデータを送信する方法は？</a>

デバイス毎に 10個のファイルを同時にアップロードできます。

## <a id="q-file-upload">アップロードされたファイルはどこに配置されますか？</a>
...
組み込みエンドポイントおよびコンシューマーグループで複数のシステムにメッセージを送信できます。

または、メッセージ ルーティング機能を使用して、組み込みイベントハブ互換エンドポイントまたはカスタムエンドポイントにメッセージを送信できます。
...
## <a id="q-sequencial-message">メッセージルーティングの順序は保証されますか？</a>

はい。イベントの順序は維持されます。

ただし、IoT Hub から Event Grid を使用した場合は、イベントの順序付けは保証されなくなります。
...
## <a id="q-device-twins">デバイスツインとは何ですか？</a>

デバイスツインとは IoT Hub で管理されるデバイスの状態に関する情報です。この情報には医科のようなものを含みます。
...
デバイスツインは IoT Hub によって、対応するデバイス IDの元で自動的に作成および削除されます。

## <a id="q-device-twin-size">デバイスツインのサイズに制限はありますか？</a>

読み取り専用の要素を除いて、メタデータ、デバイスの構成、デバイスの望ましい状態、デバイスの実際の状態の各合計値が 8KB です。

8KB の制限値を超えたデータがあると、IoT Hub はすべての操作に対してエラーを返します。

## <a id="q-partitions">IoT Hub を作成する際に設定する Device-to-cloudパーティションとは何ですか？</a>

device-to-cloud メッセージの同時閲覧者数です。ほとんどのハブでは既定値の 4 で構いません。

## <a id="q-distribute-tracing">メッセージがシステム全体でどれくらいの所要時間で配信されたか調べるにはどうしたらいいですか？</a>

Distributed Tracing の使用を検討して下さい。
...
このような標準のオプション以外にも、サービス利用者がリージョン間の高可用性を担保する設計／実装することもできます。たとえば、IoT Hub のリージョン間での HA クラスタ運用や、IoT Edge によるエッジ環境でのデータキャッシングやIoT Edge モジュールによるオフラインサービス等です。

なお、フェイルオーバーで IoT Hub の接続 IP アドレスが変更されます。したがって、クライアント側で IP アドレスのキャッシュを行わないでください。

## <a id="q-rpo">IoT Hub の障害によるデータの損失は発生しますか？</a>
...
## <a id="q-worldwide">私のデバイスは世界中で使われる予定です、何に気をつけたらいいですか？</a>

リージョンをまたがった同一の IoT Hub は現状では提供されていないので、複数のリージョンで IoT Hub を運用し、それらのメッセージを集約する必要がでてきます。

- デバイスに近いリージョンに IoT Hub を配置する
- そのリージョンで使用する IoT Hub の機能が、違うリージョンでも提供されていることを確認する
- デバイスのリージョン間の移動を伴うのであれば、デバイス情報の複製を行う。
- デバイスからのメッセージを利用するサービスのリージョンへディスパッチするために、メッセージのルーティングを活用する
- クラウドからデバイスへの送信は、リージョン間の通信をうまく扱えるように設計する
- など

特にリージョンをまたがったデバイスの管理を効率的に行うために、デバイスプロビジョニングサービスの使用を検討してください。
...
## <a id="q-resources">IoT Hub を学習するための良いリソースはありますか？</a>

Azure アカウントを持っており、IoT Hub の作成などの操作ができる環境にあれば、 Microsoft が公開している [IoT Hub のドキュメント](https://docs.microsoft.com/ja-jp/azure/iot-hub/) のチュートリアルをなぞってみるとよいでしょう。ただし、チュートリアルを完全にこなそうとすると、Azure アカウント以外にも以下のアカウントが必要になります。
